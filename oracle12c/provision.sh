...
echo "#https://docs.oracle.com/database/121/LADBI/app_manual.htm#LADBI7866"  >> /etc/sysctl.conf


echo "kernel.semmsl = 250" >> /etc/sysctl.conf
echo "kernel.semmns = 32000" >> /etc/sysctl.conf
echo "kernel.semopm = 100" >> /etc/sysctl.conf
echo "kernel.semmni = 128" >> /etc/sysctl.conf
echo "kernel.shmmax= 778260480 #Half the size of physical memory in bytes " >> /etc/sysctl.conf
echo "kernel.shmall= 2097152 # SGA*1024*1024*1024/4096" >> /etc/sysctl.conf  
echo "kernel.shmmni = 4096" >> /etc/sysctl.conf
echo "kernel.panic_on_oops = 1" >> /etc/sysctl.conf 

echo "fs.file-max = 6815744" >> /etc/sysctl.conf
echo "fs.aio-max-nr = 1048576	#Note: This value limits concurrent outstanding requests and should be set to avoid I/O subsystem failures." >> /etc/sysctl.conf
...
echo "oracle hard stack 20480" >> /etc/security/limits.conf
...
echo '		XAuthLocation /bin/xauth' >> /etc/ssh/ssh_config
